...
def _reajustes(contrato: Contrato) -> list:
    """Reajustes concluídos, do mais antigo para o mais recente."""
...
def preco_em(contrato: Contrato, item: ItemContrato, competencia: date) -> Decimal:
    """Preço unitário do item válido na competência informada."""
    # Todas as linhas deste item em reajustes concluídos, com o mês a partir do qual valem
    ...
    # O último reajuste já em vigor na competência; se nenhum ainda vigorava, o preço anterior ao 1º
...
def valor_global_vigencia(
    ...
    def preco(item: ItemContrato, mes: date) -> Decimal:
        """Preço do mês: o simulado (se houver para o item e o mês) ou o vigente."""
    ...
    for item in contrato.itens:
        # Item contínuo: soma mês a mês (com pró-rata nos meses parciais, se o item usar)
        if item.tipo == "continuo":
            ...
            # Item sob demanda: soma os apontamentos e completa com o saldo que não foi apontado
            apontado = Decimal(0)
            for mes in meses:
